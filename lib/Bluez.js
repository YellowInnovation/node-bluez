...
class Bluez extends EventEmitter {
    ...
    async init() {
        ...
        this.bus.on('signal', this.signalHandler.bind(this));
    }
    
    signalHandler(uniqueBusName, sender, objectPath, interfaceName, signalName, args) {
        if (signalName === 'PropertiesChanged' && objectPath.includes('bluez')) {
            if (Array.isArray(args) && args.length > 1) {
                let [[key, value]] = Object.entries(args[1]);
                let addr = objectPath.substr('/org/bluez/hci0/dev_'.length, 17).replace(/_/g, ':');
                switch (args[0]) {
                    case 'org.bluez.Device1':
                        this.emit('device_property', addr, key, value);
                        break;
                    case 'org.bluez.MediaPlayer1':
                        this.emit('player_property', addr, key, value);
                        break;
                    case 'org.bluez.MediaTransport1':
                        this.emit('transport_property', addr, key, value);
                        break;
                    case 'org.bluez.MediaControl1':
                        this.emit('control_property', addr, key, value);
                        break;
                }
            }
        }
    ...
    close() {
        Object.keys(this.adapter).forEach((key) => {
            if (this.adapter[key].removeAllListeners) {
                this.adapter[key].removeAllListeners();
            }
            if (this.adapter[key].destroy) {
                this.adapter[key].destroy();
            }
        });
        this.objectManager.removeAllListeners('InterfacesAdded');
        this.objectManager.removeAllListeners('InterfacesRemoved');
        ...
        if (this.userServiceObject) {
            ...
            this.userServiceObject = undefined;
        }
