...
class Device extends EventEmitter {
    ...
    initEvents()
    {
        if (this._DBusObject.listenerCount('signal') !== 0)
            return;
        this._DBusObject.on('signal', (uniqueBusName, sender, objectPath, interfaceName, signalName, args) => {

            if(objectPath == this._interface.objectPath)
            {
                if(Array.isArray(args))
                {
                    if(args.length > 1 && args[0] == 'org.bluez.Device1')
                    {
